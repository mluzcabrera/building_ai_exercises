...
def main(X_train, X_test, y_train, y_test):
    ...
    for i, test_item in enumerate(X_test):
        ...
        # indices that would sort by distance (ascending)
        ...
        # majority vote (tie-breaker = smallest class label)
        y_predict[i] = np.bincount(k_labels).argmax()

        # (optional) store segments to each of the k neighbors for plotting
        for j in k_idx:
